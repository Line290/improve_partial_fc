...
#     reid_fc1 = mx.sym.FullyConnected(data=final_flatten, num_hidden=512, name=namepre+"_fc1", 
#                                    weight=args['finalfc']['weight'], bias=args['finalfc']['bias'])
...
#     probs = np.exp(log_probs)
    ...
    loss = -np.sum(log_sparse_probs[y_true.reshape(-1), np.arange(N)]) / N
    d_sparse_score = sparse_probs.copy()
#     sparse_probs[y_true.reshape(-1), np.arange(N)] += -1
    d_sparse_score[y_true.reshape(-1), np.arange(N)] += -1
    # rescale gradient
#     sparse_probs /= N
    return loss, d_sparse_score

class mixModule(object):
    def __init__(self, symbol, context, handle, data_shape, proxy_Z, K, param_prefix, load_paramidx, hardratio):
...
#         self.history = np.zeros((self.C, self.M), np.float32)
        ...
        self.acc = None
        self.v_pre = np.zeros((self.C, self.M), np.float32)
...
#         param_name = SNAP_SHOT_PATH + '%s-%04d.params' % (prefix, epoch)
    ...
    def update(self, data_batch):
        ...
        X_GPU = self.mod.get_outputs()[0]
        X_GPU.wait_to_read()
        self.X = X_GPU.asnumpy()
#         np.save('feat_final', self.X)
#         self.X = np.load('../feat_final.npy')[-10:]
...
#         print 'label : ', y_true[:10].reshape(-1)
        self.W = self.W / (np.sqrt(np.sum(self.W**2, axis=0, keepdims=True)) + 10**-36)
#         print 'self.W : ', self.W[0,:10]
#         self.score = self.X.dot(self.W).T
        ...
        self.acc = np.mean(np.argmax(self.score, axis=0)==y_true.reshape(-1))
        self.ang = self.score[y_true.reshape(-1), self.idx]
#         print 'score true : ',self.ang
#         print 'max score : ', np.max(self.score, axis=1)
        self.score[y_true.reshape(-1), self.idx] += np.log(self.hardratio)
        
# #         self.score = self.score - np.max(self.score, axis=0, keepdims=True)
        self.score = np.exp(self.score)
        self.score = self.score / np.sum(self.score, axis=0, keepdims=True)
# # #         print self.score.shape

        y_true_score = self.score[y_true.reshape(-1), self.idx]
#         self.score[y_true.reshape(-1), self.idx] = self.score.min(axis = 0)
        self.score[y_true.reshape(-1), self.idx] = -1000
        TopK_idx = np.argpartition(self.score, -self.K, axis=0)[-self.K:,:]
        self.score[y_true.reshape(-1), self.idx] = y_true_score
        TopK_idx = np.vstack((TopK_idx, y_true.reshape(1,-1)))
        TopK_score = self.score[TopK_idx, self.idx]
        y_true_reform = np.array([self.K]*self.N).reshape(-1,1)
#         print 'TopK_score max ind : ', np.argmax(TopK_score, axis=1)

        
        self.loss, d_TopK_score = softmax_loss(TopK_score, y_true_reform)
#         self.loss, d_TopK_score = softmax_loss(self.score, y_true)
        
        self.score[:] = 0
        self.score[TopK_idx, self.idx] = d_TopK_score
        

        csc_d_score = scipy.sparse.csc_matrix(self.score) # M*N



        d_proxy_Z = csc_d_score.dot(self.X).T #C*M  
#         d_proxy_Z = d_TopK_score.dot(self.X).T

#         d_feat_final = self.W.dot(d_TopK_score).T
#         d_feat_final = mx.nd.dot(csr_d_score, proxy_Z.T)
        d_feat_final = csc_d_score.T.dot(self.W.T)
#         print 'd_feat_final :', d_feat_final[0,:10]
        ...
        mom = self.mod._optimizer.momentum
        # backprop
        d_feat_final.wait_to_read()
#         print 'd_feat_final :', d_feat_final[0,:10]
#         print 'd_feat_final : ', d_feat_final[0,:10]
#         print 'shape : ', d_feat_final.shape
        self.mod.backward([d_feat_final])
        # update module
        self.mod.update()
#         time.sleep(3)
        # update W
#         self.v_pre = self.v_pre * mom - lr * d_proxy_Z
            
#         self.W[:] += self.v_pre
        self.W += -lr * d_proxy_Z / self.N
...
#         if num_iter%1000 == 0:
    ...
    def get_ang(self, radius):
        self.ang /= radius
        return np.mean(np.arccos(self.ang)/np.pi*180)